...
class CharacterDisplayWidget(QWidget):
    def __init__(self, character, dice_manager, output_buffer,):
        super().__init__()
        ...
        for talent_group in character.talent_groups:
            if talent_group.name == "Base":
                base_talent_layout = TalentDisplayLayout(output_buffer, dice_manager, character, talent_group)
                character_base_and_info_layout.addLayout(base_talent_layout)
            else:
                talent_group_layout = TalentDisplayLayout(output_buffer, dice_manager, character, talent_group)
